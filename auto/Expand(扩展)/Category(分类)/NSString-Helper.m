...
//RegularExpression
static  NSString * RE_Email = @"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
static  NSString * RE_IDC = @"^(\\d{14}|\\d{17})(\\d|[xX])$";

static  NSString * RE_PlateNumber = @"^[\u4e00-\u9fff]{1}[a-zA-Z]{1}[-][a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-\u9fff]$";//其中\u4e00-\u9fa5表示unicode编码中汉字已编码部分，\u9fa5-\u9fff是保留部分，将来可能会添加
static  NSString * RE_Url = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
static  NSString * RE_Chinese = @"^[\u4e00-\u9fa5]+$";
static  NSString * RE_Postalcode = @"^[0-8]\\d{5}(?!\\d)$";
