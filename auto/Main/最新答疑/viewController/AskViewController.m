...
-(void)createUI{
    ...
    titleField = [LHController createTextFieldWithFrame:CGRectMake(LEFT+20, 10, WIDTH-15*2-20, H)  andBGImageName:nil andPlaceholder:@"输入标题(最多20个字)" andTextFont:B+1 andSmallImageName:nil andDelegate:self];
    ...
    UILabel *labelLeftContent = [self createLabel:@"*" andFont:B andFram:CGRectMake(11, 5, 10, H)];
    ...
    PlaceholderLebel = [LHController createLabelWithFrame:CGRectMake(25, 7, 80, 20) Font:B-2 Bold:NO TextColor:colorLightGray Text:@"输入内容"];
...
-(void)submitData{
    ...
    hud.labelText = @"正在提交...";
    NSDictionary *dict = @{@"id":@"0",@"uid":[CZWManager manager].userID,@"title":titleField.text,@"content":contentView.text,@"origin":appOrigin};
    [HttpRequest POST:[URLFile url_editZJDY] parameters:dict success:^(id responseObject) {
        button.enabled = YES;
...
-(UILabel *)createLabel:(NSString *)name andFont:(CGFloat)size andFram:(CGRect)fram{
    ...
    label.text = name;
    label.textColor = colorOrangeRed;
