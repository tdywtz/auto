...
-(void)loadData{
    ...
    NSString *url = [URLFile url_plWithId:_cid type:type page:_count sum:10];
...
-(void)submitComment:(NSString *)content{
    if (![LHController judegmentSpaceChar:content]) {
        if (commentORanswer == commentTypeComment) {
            ...
            [ProgressHUD showProgressHUDInView:self.parentViewController.view
                                      withText:@"评论内容不能为空"
                                    afterDelay:1];
    ...
    if (model.initialModel) {
        ...
        [initialCell getPid:^(NSString *pid) {
            if ([CZWManager manager].isLogin) {
                [weakSelf pushComment:pid text:@"回复"];
            }else{
