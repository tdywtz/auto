...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(imageModelNoti) name:ComplainImageModelNotification object:nil];
}
...
-(void)loadUserInfo{
    ...
    NSString *url = [URLFile url_userWithUid:CZWManagerInstance.userID];
...
- (void)loadDataUpdate{
    ...
    NSString *url = [URLFile url_tsdetailWithCpid:self.Cpid];
    [HttpRequest GET:url success:^(id responseObject) {
     [MBProgressHUD hideHUDForView:self.view animated:YES];
...
//    return;
    ...
    __weak __typeof(self)weakSelf = self;
    [HttpRequest POST:[URLFile url_addcomplain] parameters:dic images:imageModel.imageArray success:^(id responseObject) {
...
- (BOOL)testInformation:(BOOL)isEqual{
    ...
    if (![LHController judegmentChar:_dataDictionary[@"Question"]]){
        [self alert:@"问题描述只能包含汉字、数字、字母、标点符号"];
        return NO;
    }
    if ([_dataDictionary[@"Question"] length] > 24) {
        [self alert:@"问题描述不能大于24个汉字"];
        return NO;
    }
...
- (void)setDataInfo{
    ...
    _dataDictionary[@"Cpid"]    = _Cpid?_Cpid:@"0";
    ...
    for (ComplainSectionModel *sectionModel in _dataArray) {
        for (ComplainBasicModel *model in sectionModel.rowModels) {
            [model setDictionaryKeyValueWithDictionary:_dataDictionary];
        }
    }
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ComplainSectionModel *sectionModel = _dataArray[indexPath.section];
    ComplainBasicModel *rowModel = sectionModel.rowModels[indexPath.row];
