...
-(void)loadDataOne{
    ...
    NSString *url = [URLFile url_complaininfoWithId:_cid];
...
- (void)setData{
    ...
    contentAtt.yy_firstLineHeadIndent = 27;
...
- (void)clickButton:(NSInteger)slected{
    if (slected == 0) {
        if ([CZWManager manager].isLogin) {
            ...
            WriteViewController *_commentView = [[WriteViewController alloc] init];
            [_commentView send:^(NSString *content) {
                [weakSelf submitComment:content];
            }];
            [self presentViewController:_commentView animated:YES completion:nil];
        }else{
             __weak __typeof(self)weakSelf = self;
