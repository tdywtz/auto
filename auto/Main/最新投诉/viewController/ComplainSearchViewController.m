...
-(void)setUrlWith:(NSString *)string andP:(NSInteger)p andS:(NSInteger)s{
    self.urlString = [URLFile url_complainListWithTitle:string p:p s:s];
}
...
-(void)createNoView{
    ...
    UILabel *label = [LHController createLabelWithFrame:CGRectMake(0, 0, 180, 20) Font:17 Bold:NO TextColor:[UIColor blackColor] Text:@"没有搜索到结果"];
...
-(NSAttributedString *)attributeSize:(NSString *)str searchStr:(NSString *)search{
    ...
    [att addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:17] range:NSMakeRange(0, att.length)];
