...
- (void)setModel:(MyComplainModel *)model{
    ...
    if (range.length) {
        [att yy_setColor:RGB_color(237, 27, 36, 1) range:range];
    }
...
-(void)loadHeaderData{
    __weak __typeof(self)weakSelf = self;
    [HttpRequest GET:[URLFile url_mytslistWithCpid:_Cpid] success:^(id responseObject) {
...
- (void)updateLayout:(BOOL)showLeft{
    self.scrollView.lh_height = self.view.lh_height - 45;
...
- (void)changeClick:(NSString *)title{
    if ([title isEqualToString:@"再次投诉"]) {
       ...
       // [self createWriteView];
        ...
        [HttpRequest GET:[URLFile url_delComNoReasonWithCpid:_Cpid] success:^(id responseObject) {
...
-(void)submitStar:(NSInteger)star{
    ...
    NSString *url = [URLFile url_complainscoreWithCpid:_Cpid score:[NSString stringWithFormat:@"%ld",star]];
    [HttpRequest GET:url success:^(id responseObject) {
