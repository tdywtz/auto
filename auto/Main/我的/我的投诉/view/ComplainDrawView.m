...
- (void)setListModels:(NSArray *)listModels{
    ...
    for (int i = 0; i < listModels.count; i ++) {
        ...
        CGSize size = [model.attribute  boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;;
...
- (void)setSteps:(NSArray *)steps{
    ...
    for (int i = 0; i < steps.count; i ++) {
        ...
        attribute.yy_font = [UIFont systemFontOfSize:13];
        if (i == steps.count - 1) {
            attribute.yy_color = _lightTextColor;
        }else{
            attribute.yy_color = _textColor;
        }
        attribute.yy_paragraphSpacing = 5;
