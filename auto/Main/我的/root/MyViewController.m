...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    nvc.alphaView.alpha = 1;
}

- (void)viewDidAppear:(BOOL)animated{
    [super viewDidAppear:animated];
    BasicNavigationController *nvc = (BasicNavigationController *)self.navigationController;
    nvc.alphaView.alpha = 0;
}
...
- (void)createTableView{
    ...
    [_tableView addTwitterCoverWithImage:[UIImage imageNamed:@"timg.jpeg"] withTopView:nil];
...
#pragma mark -更新数目
-(void)updateNumber{
    if (CZWManagerInstance.isLogin == NO) {
        return;
    }
    NSString *url = [URLFile url_personalCountWithUid:[CZWManager manager].userID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0){
            ...
            label.text = [NSString stringWithFormat:@"%ld",[FavouriteModel findAll].count];
        }
    }
